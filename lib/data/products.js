...
export const getAllProducts = async (limit = 50, offset = 0) => {
  // add pagination params if not already
  try {
    const { data, error } = await supabase
      .from("products")
      .select(
      ...
      .order("random_seed", { ascending: true }) // consistent within one "shuffle period"
    ...
    // Optional client-side double-check (can be removed if RLS is solid)
    const filteredData = (data || []).filter(
      (product) => product.profiles?.is_active !== false
    );
...
export const getCharityProducts = async (limit = 50, offset = 0) => {
  // add pagination params if not already
  try {
    const { data, error } = await supabase
      .from("products")
...
export const getRandomProducts = async (limit = 20) => {
  ...
  let { data, error } = await supabase
    ...
    .select(
      ...
      *,
      categories (id, name),
      profiles!supplier_id (id, full_name, is_active)
    `
    )
    .eq("is_active", true)
    .eq("profiles.is_active", true)
