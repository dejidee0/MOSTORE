// lib/toast.js - Updated Toast Notification System
"use client";
import React, {
  createContext,
  useContext,
...
export const ToastProvider = ({ children }) => {
  ...
  const addToast = useCallback((message, type = "info", options = {}) => {
    ...
    const toast = {
      ...
      isCustom: typeof message !== "string",
    };
...
const Toast = ({ toast, onRemove }) => {
  const { message, type, isCustom, onClick } = toast;

  const getToastStyles = (type) => {
    const baseStyles =
      "flex items-start p-4 rounded-lg shadow-lg border-l-4 min-w-80 max-w-md pointer-events-auto";
    ...
    switch (type) {
      ...
      case "message":
        return `${baseStyles} bg-white border-orange-400 text-gray-800 cursor-pointer hover:shadow-xl transition-shadow`;
      case "info":
      default:
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "message":
        return null; // Custom content will have its own icon
      case "info":
      default:
        return (
          <svg
  ...
  return (
    ...
    >
      {!isCustom && getIcon(type)}
      <div className={`${!isCustom ? "ml-3" : ""} flex-1`}>
        {isCustom ? message : <p className="text-sm font-medium">{message}</p>}
      </div>
      <button
        onClick={(e) => {
          e.stopPropagation();
